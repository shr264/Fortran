...
integer :: repeats, directiontomove, end1, end2, timesteps, boundary, initialdist
...
!initialdist = 0 imples center, = 1 imples random

namelist /data/ initialdist, boundary, timesteps, repeats, nspaces, maxparticlesperspace, nspecies 

open(9, file = 'input.txt')
read(*,nml=data) ! Will read namelist from standard input
write(*,*) 'Read the following inputs:'
write(*,nml=data)
close(9)

if (initialdist == 0) then
   avgparticlesperspace = nspaces*maxparticlesperspace
elseif (initialdist == 1) then
   avgparticlesperspace = int(0.5*maxparticlesperspace)
endif
...
Initialize: if(initialdist == 1) then
do count1 = 1,nparticles
...
enddo 
elseif (initialdist == 0) then
   positiontomove = int(nspaces/2)
do count1 = 1,nparticles
   call random_number(rndm1)
   particletomove = int(rndm1*nparticles) + 1
   species = pickspecies(initialspecies,nspecies)
